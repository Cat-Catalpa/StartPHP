...
class ThrowError{
    function __construct($errfile,$errline,$errorcode="System Error",$customSuffix = "",$errtitle = "",$errno=2){
        global $lang,$env,$hasBeenRun;
        $debug = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS);
        if((isset($env['error_auto_clean']) && $env['error_auto_clean'])) {
                ob_clean();
        }
        elseif((!isset($env['error_auto_clean']) && $env['debug_mode'])) {
                ob_clean();
        }
        ...
        if ($env['save_error_log']) {
            \Cache::errorOut($errDetail);
        }
        ...
        if ($env['send_error_log']) {
            $to = $env['developer_email'];         // 邮件接收者
